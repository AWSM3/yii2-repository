...
trait SqlArRepositoryTrait
{
    ...
    public function findOneById($id, $returnArray = false)
    {
        ...
        return $this->query->where([$this->primaryKey => $id])->limit(1)->one();
    }
    ...
    {
        if (!$criteria) {
            throw new RepositoryException('$criteria must defined');
        }
    ...
    {
        $entity = $this->query->where([$this->primaryKey => $id])->limit(1)->one();
